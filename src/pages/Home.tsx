...
export default function Home() {
  ...
  const colorDistribution = useMemo(() => {
    ...
    return colorTags.map((tag) => ({
      ...tag,
      count: products.filter((p) =>
        p.tagList?.some((t: { id: number }) => t.id === tag.id)
      ).length,
    }));
  }, [allTags, products]);
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA]">
          ...
          <div className="flex items-center gap-1 overflow-x-auto pb-2 scrollbar-hide">
            {colorDistribution
              ...
              .map((color) => (
                <div
                  key={color.id}
                  className="flex-shrink-0 flex items-center gap-1 px-2.5 py-1 rounded-full bg-white border border-gray-100 text-xs"
                  title={`${color.name}: ${color.count} 瓶`}
                >
                  <span
                    className="w-2.5 h-2.5 rounded-full"
                    style={{ backgroundColor: color.colorHex || "#ccc" }}
                  />
                  <span className="text-gray-600">{color.count}</span>
                </div>
              ))}
          </div>
        )}
      ...
      <div className="sm:hidden fixed bottom-4 left-4 right-4 flex gap-2">
        <button
          onClick={() => navigate("/upload")}
          className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-pink-50 text-pink-600 rounded-xl text-sm font-medium border border-pink-200 shadow-lg shadow-pink-100"
        >
          <Upload className="w-4 h-4" />
          上传订单
        </button>
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-pink-500 text-white rounded-xl text-sm font-medium shadow-lg shadow-pink-200"
        >
          <Plus className="w-4 h-4" />
          添加产品
        </button>
